...
public abstract class BaseDashboardView extends View {
    ...
    //数值%符号画笔
    protected TextPaint mPaintValueSmall;
    ...
    // 默认刻度文字
    private final static int[] DEFAULT_CALIBRATION_NUMBER = new int[]{30, 50, 70, 80, 90};
    private final static String[] DEFAULT_CALIBRATION_BETWEEN = new String[]{"较差", "中等", "良好", "优秀"};
    // 默认刻度数量
    public final static int DEFAULT_LARGE_BETWEEN_CALIBRATION_NUMBER = 3;
    // 默认控件大小
    ...
    // 数值画笔属性
    private final static float DEFAULT_VALUE_TEXT_SIZE = 52f;
    ...
    //圆环刻度颜色
    protected final int[] colors ={Color.parseColor("#8ED17A"),Color.parseColor("#FFB44D")
            ,Color.parseColor("#11C3FF"),Color.parseColor("#FF4B25")};

    //圆环刻度颜色
    protected final int[] colorsEnd ={Color.parseColor("#CAD972"),Color.parseColor("#FFDF67")
            ,Color.parseColor("#00E1FF"),Color.parseColor("#FFB059")};
    ...
    private void init(){
        ...
        mPaintValue.setDither(true);
        mPaintValue.setAntiAlias(true);

        mPaintValueSmall = new TextPaint(Paint.ANTI_ALIAS_FLAG);
        mPaintValueSmall.setTextAlign(Paint.Align.CENTER);
        mPaintValueSmall.setTextSize(sp2px(DEFAULT_VALUE_TEXT_SIZE));
        mPaintValueSmall.setColor(DEFAULT_VALUE_TEXT_COLOR);
        mPaintValueSmall.setAntiAlias(true);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        drawKDArc(canvas, mArcStartAngle, mArcSweepAngle);
    ...
    /**
     * 设置圆环角度
     * @param arcStartAngle 弧开始的角度，以X轴正方向为0度
     * @param arcSweepAngle 弧持续的角度
     */
